...
vmap #    :call PythonToggleCommentSelection()<CR>

map  <s-up>    :call PythonNextLine(-1)<CR>
map  <s-down>  :call PythonNextLine(1)<CR>
...
" If the first line is commented, uncomment the selection, otherwise
" noncomment the whole thing
function! PythonToggleCommentSelection() range
  if match(getline(a:firstline), "^[ \t]*#") == -1
    call PythonDoCommentSelection(a:firstline, a:lastline)
  else
    call PythonDoUncommentSelection(a:firstline, a:lastline)
  endif
endfunction
...
" the block
function! PythonDoCommentSelection(first, last)  
  let commentString = "#"
  let cl = a:first
...
function! PythonDoUncommentSelection(first, last)  
...
function! PythonUncommentSelection()  range
  call PythonUncommentSelection(a:firstline, a:lastline)
endfunction
